...
- Windows 11 Pro, bash shell in Claude Code
- `python` (not `python3`)
- Global .NET tools: `dotnet-coverage`, `dotnet-reportgenerator-globaltool`

## Diff Coverage Analysis (.NET)
For validating test coverage on branch changes, use the `/diff-coverage` skill. Prerequisites: `dotnet-coverage` and `dotnet-reportgenerator-globaltool` global tools.
...
## Graphify
To convert any input (code, docs, papers, images) into a knowledge graph, use the `/graphify` skill.
