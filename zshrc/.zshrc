...
# Override oh-my-zsh's `:completion:*:*:*:*:*' menu select with the same specific pattern so most-specific-wins flips back to no popup.
zstyle ':completion:*:*:*:*:*' menu no
# In command position try cwd files first (so `arch<Tab>` hits `archsetup.sh`); for arguments use only _complete so per-command completers like _git still run.
zstyle -e ':completion:*' completer '
    if [[ $CURRENT -le 1 ]]; then
        reply=(_files _complete)
    else
        reply=(_complete)
    fi'
